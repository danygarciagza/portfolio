...
(() => {
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Encode at 2× the slot width for retina, but never below an 1100px
      // floor — small slots (e.g. a 130px ID photo) would otherwise store a
      // soft ~260px image that looks pixelated on a face / high-DPI screen.
      const cap = Math.min(MAX_DIM, Math.max(1100, Math.round(targetW * 2)) || MAX_DIM);
