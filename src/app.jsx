...
function Trinket({ kind, floating }) {
  ...
  // turntable — cut-out object, freely positioned + sized via tweaks
  return (
    <div className={"ledge-trinket turntable" + (floating ? " floating" : "")} aria-hidden="true">
      <img className="trinket-img" src="uploads/record-player.png" alt="" />
    </div>
  );
}
    ...
    <div className="page-wrap view-enter">
      ...
      <div className={"shelf-wall" + (editCovers ? " editing" : "")}>
        {SHELVES.map(({ catId, props, booksSide }) => {
          ...
          const booksEl = (
            <div className="lb-cluster" key="books">
              {items.map((p, i) => {
                ...
                return (
                  ...
                  >
                    <div className="lb-cover">
                      <image-slot id={`card-${p.slug}`} shape="rect" fit="cover" placeholder={tx(p.title, lang)}></image-slot>
                      {titlePos !== "off" && (
          ...
          return (
            <div key={catId} className={"ledge-row row-" + catId + (glow === catId ? " ledge-focus" : "")}>
              ...
              <div className="ledge-books">
                {catId === "furniture"
                  ? [<div className="lb-cluster props" key="props"><Trinket kind="cat" /><Trinket kind="turntable" /></div>, booksEl]
