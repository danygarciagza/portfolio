      ...
      <div className="detail">
        ...
        <div className="detail-info">
          ...
          <div className="detail-nav">
            <div className="dn" onClick={() => go("project", { cat: cat.id, slug: prev.slug })}>
              <span className="lbl">← {tx(T.prev, lang)}</span>
              <span className="nm display">{tx(prev.title, lang)}</span>
            </div>
            <div className="dn" style={{ textAlign: "right" }} onClick={() => go("project", { cat: cat.id, slug: next.slug })}>
              <span className="lbl">{tx(T.next, lang)} →</span>
              <span className="nm display">{tx(next.title, lang)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="page-wrap view-enter">
      ...
      <div style={{ maxWidth: 640, margin: "8px auto 0", textAlign: "center" }}>
        <h1 className="display" style={{ fontWeight: 800, fontSize: "clamp(40px,7vw,76px)", letterSpacing: "-.03em", margin: "0 0 6px" }}>
          {lang === "es" ? "Hablemos" : "Let's talk"}<span style={{ color: "var(--accent)" }}>.</span>
        </h1>
        <p style={{ fontSize: 18, color: "var(--ink-soft)", lineHeight: 1.6, maxWidth: 460, margin: "0 auto 34px" }}>
          {lang === "es" ?
